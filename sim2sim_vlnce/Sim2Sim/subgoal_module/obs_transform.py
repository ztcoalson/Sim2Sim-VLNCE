...
class ResNetAllEncoderEfficient(ObservationTransformer):
    ...
    def encode(self, frames: Tensor, gflops_tracker: Dict, frame_episode_mapping: List[int]) -> Tensor:
        ...
        for i in range(num_frames):
            ...
            gflops_tracker[frame_episode_mapping[i]]["image_encoder"] += flops / 1e9
...
class SubgoalModuleEfficient(SubgoalModuleABC):
    ...
    def __init__(
        ...
        nms_thresh:
        """
        super(SubgoalModuleEfficient, self).__init__()
        self.max_candidates = max_candidates + 1
        self.sigma = nms_sigma
        self.thresh = nms_thresh
        self.angle_feature_size = angle_feature_size
    ...
    def predict_subgoal_img(
        ...
        if use_ground_truth:
            ...
            prepped_scan = self.prep_scan_for_net(centered_ro_map)
            
            logits = None
            for i in range(len(episode_ids)):
                _prepped_scan = prepped_scan[i].unsqueeze(0)
                _centered_feats = centered_feats[i].unsqueeze(0)

                _logits, flops, _ = profile(self.unet, inputs=(_prepped_scan, _centered_feats), verbose=False)

                if logits is None:
                    logits = torch.zeros((len(episode_ids), *_logits[0].shape)).to(_logits.device)
                
                logits[i] = _logits[0].detach()

                gflops_tracker[episode_ids[i]]["subgoal_module"] += flops / 1e9
...
class SubgoalModuleScanOnlyEfficient(SubgoalModuleABC):
    ...
    def __init__(
        self,
        max_candidates: int,
        scan_only_net_weights_file: str,
        scan_only_net_channels: int,
        nms_sigma: float,
        nms_thresh: float,
        angle_feature_size: int,
        remove_rgb: bool,
        ...
        max_candidates: max number of subgoal candidates limited via NMS.
            selfmax_candidates includes STOP.
        scan_only_net_weights_file: located of re-trained weights for the ScanOnlyNet.
        scan_only_net_channels: number of internal channels in the ScanOnlyNet.
        nms_sigma:
        nms_thresh:
        """
        super(SubgoalModuleScanOnlyEfficient, self).__init__()
        self.max_candidates = max_candidates + 1
        self.sigma = nms_sigma
        self.thresh = nms_thresh
        self.angle_feature_size = angle_feature_size
        ...
        self.range_bin_width = range_bin_width
        self.heading_bin_width = heading_bin_width
        self.scan_only_net = self.load_subgoal_module(
            scan_only_net_weights_file, scan_only_net_channels, device
        )
        self.range_correction = range_correction
        self.heading_correction = heading_correction
    ...
    def forward(
        ...
        # map candidates to features and relative polar coordinates
        feature_size = 2048 + self.angle_feature_size
        candidate_features = torch.zeros(
            batch_size, self.max_candidates, feature_size, device=device
        )
        candidate_coordinates = torch.zeros(
    ...
    def predict_subgoal_img(
        ...
        for i in range(len(episode_ids)):
            _prepped_scan = prepped_scan[i].unsqueeze(0)
            _logits, flops, _ = profile(self.scan_only_net, inputs=(_prepped_scan,), verbose=False)
    ...
    def encode_feats_from_pose(
        ...
        feats = torch.zeros(
            rel_heading.shape[0],
            feature_size + angle_feature_size,
            device=rel_heading.device,
        )
        feats[:, feature_size :] = vln_style_angle_features(
            rel_heading, elevation, angle_feature_size
        )
        for i in range(len(rel_heading_lst)):
            e = round(elevation_lst[i], 2)
    ...
    def load_subgoal_module(
        ...
        scan_only_net = ScanOnlyNet(n_channels=2, n_classes=1, ch=scan_only_net_channels).to(device)
        if scan_only_net_weights_file != "":
            state_dict = torch.load(scan_only_net_weights_file, map_location="cpu")
            scan_only_net.load_state_dict(state_dict)

        scan_only_net.eval()
        return scan_only_net
    ...
    def from_config(cls, config: Config):
        ...
        return cls(
            cfg.max_candidates,
            cfg.scan_only_net_weights_file,
            cfg.scan_only_net_channels,
            cfg.subgoal_nms_sigma,
            cfg.subgoal_nms_thresh,
            cfg.angle_feature_size,
            cfg.remove_rgb_feats,
...
class SubgoalModuleEfficientTest(SubgoalModuleABC):
    ...
    def __init__(
        ...
        nms_thresh:
        """
        super(SubgoalModuleEfficientTest, self).__init__()
        self.max_candidates = max_candidates + 1
        self.sigma = nms_sigma
        self.thresh = nms_thresh
        self.angle_feature_size = angle_feature_size
    ...
    def predict_subgoal_img(
        ...
        if use_ground_truth:
            ...
            for i in range(len(episode_ids)):
                ...
                _logits, flops, _ = profile(self.unet, inputs=(_prepped_scan,), verbose=False)
    ...
    def load_subgoal_module(
        ...
        unet = ScanOnlyNet(n_channels=2, n_classes=1, ch=unet_channels).to(device)
        if unet_weights_file != "":
            state_dict = torch.load(unet_weights_file, map_location="cpu")
            unet.load_state_dict(state_dict)
